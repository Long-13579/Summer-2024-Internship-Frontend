...
export default function ShowTime({ film }) {
  ...
  useEffect(() => {
    ...
    if (film?.dateList?.length > 0) {
      setSelectedDate(film.dateList[0])
    }

    if (film?.provinceList?.length > 0) {
      setSelectedProvinceCity(film.provinceList[0])
    }
  }, [film])

  useEffect(() => {
    fetchCinemaShowOfFilm(film?.info?.id, selectedDate, selectedProvinceCity?.id)
      ...
      <div className='mt-10'>
        <div className='flex items-center justify-between'>
          ...
          <div
            ...
            ref={listRef}
          >
            <img src={IconLocation} alt='location' className='h-[24px] w-[24px]' />
        ...
        <div className='mt-4 flex flex-col gap-4'>
          ...
          {cinemaShows &&
            cinemaShows.map((cinemaShow) => <CinemaShow key={cinemaShow.cinemaId} cinemaShow={cinemaShow} />)}
        </div>
      </div>
    </div>
  )
