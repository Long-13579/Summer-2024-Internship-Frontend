...
export default function Button({ title, icon, extraClass = '', imgClass = '', txtClass = '', ...rest }) {
  return (
    <div
      className={cn(
        'group relative flex items-center gap-2 overflow-hidden rounded-md bg-yellow-custom-700 px-4 py-2 text-black transition-colors duration-1000 ease-in-out hover:cursor-pointer hover:text-white',
        extraClass
      )}
      {...rest}
    >
