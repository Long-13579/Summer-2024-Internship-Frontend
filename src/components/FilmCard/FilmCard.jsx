...
import { setFilmIdOfShowedTrailer, toggleMovieOverlay } from '@/redux/slices/ui'
import MovieOverlay from '@/components/MovieOverlay'

const FilmCard = ({ film }) => {
  const { id, title, poster, ageRating, format, category, duration, nation, subtitle, trailerLink } = film
  const [isDragging, setIsDragging] = useState(false)
  const dispatch = useDispatch()
  const { isShowMovieOverlay, filmIdOfShowedTrailer } = useSelector((state) => state.ui)
  ...
  const handleShowMovieOverlay = () => {
    dispatch(setFilmIdOfShowedTrailer(id))
    ...
    <>
      ...
      >
        ...
        <div className='mt-6 flex items-center justify-between'>
          <div
            className='gap flex h-full items-center gap-2 hover:cursor-pointer hover:text-yellow-custom-700'
      ...
      {isShowMovieOverlay && filmIdOfShowedTrailer === id && <MovieOverlay url={trailerLink} />}
    </>
  )
}
